...
if (isset($_GET['id'])) {
    ...
    if ($poll) {
        ...
        if (isset($_POST['poll_answer'])) {
            ...
            $stmt->execute
            ([ $_POST['poll_answer']]);
            // $stmt = $pdo->prepare('SELECT * FROM user_logs WHERE poll_id = ?');
            // $stmt->execute([ $_GET['id'] ]);
            $stmt = $pdo->prepare('UPDATE user_logs SET votes = votes + 1 WHERE poll_id = ?');
...
<div class="content poll-vote">
    ...
    <form action="vote.php?id=<?=$_GET['id']?>" method="post">
        ...
        <label>
            <input type="checkbox" name="poll_answer" value="<?=$poll_answers[$i]['id']?>"<?=$i == 0 ? ' checked' : ''?>>
